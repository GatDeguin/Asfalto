...
import { createCockpitLayoutEndpoint } from './src/server/cockpit-layout-store.mjs';
import {createLightingPresetEndpoint} from './src/server/lighting-preset-store.mjs';
...
export async function main() {
  ...
  const cockpitDataDirectory=await prepareV7DataDirectory({sourceDirectory:path.resolve(root,'assets','configuration'),targetDirectory:path.resolve(root,'.local-data','v7')});
  let instance = null;
  let lastError = null;
