...
export function prepareSoundscapeBanks(context,{seed=6147,signal,timeoutMs=45000,createWorker=()=>new Worker(new URL('./soundscape-banks-worker.mjs?v=17dd7930d2372e0b',import.meta.url),{type:'module'})}={}){
 ...
 return new Promise((resolve,reject)=>{
  let worker,finished=false,timer;
  const finish=(error,value)=>{if(finished)return;finished=true;clearTimeout(timer);signal?.removeEventListener('abort',abort);worker?.terminate();if(error)reject(error);else{entries.set(seed,value);resolve(value);}};
  const abort=()=>finish(signal.reason||new DOMException('Aborted','AbortError'));
  timer=setTimeout(()=>finish(new DOMException('Soundscape preparation timed out','TimeoutError')),timeoutMs);
