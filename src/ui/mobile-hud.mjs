...
export function createMobileHud({doc=document,win=window,update,readSpeedKmh=physicalSpeedKmh,now=()=>performance.now(),frames=getFrameScheduler()}={}){
 let disposed=false,cached=false,updates=0;const body=doc.body,hosts=[...doc.querySelectorAll('.chevy-hud-host')],previous=hosts.map(h=>h.dataset.renderState);
 ...
 function tick(){if(!visible())return;update(now());const velocity=readSpeedKmh(),label=Number.isFinite(velocity)?String(Math.round(Math.abs(velocity))):'—';if(speedValue.textContent!==label)speedValue.textContent=label;updates++;}
 const onHide=event=>{cached=!!event.persisted;if(!event.persisted)dispose();},onShow=event=>{if(event.persisted){cached=false;frames.wake();}};
 const stopFrame=frames.subscribe('hud-mobile',tick,{hz:20,enabled:visible});win.addEventListener('pagehide',onHide);win.addEventListener('pageshow',onShow);
 function dispose(){if(disposed)return false;disposed=true;stopFrame();win.removeEventListener('pagehide',onHide);win.removeEventListener('pageshow',onShow);hosts.forEach((h,i)=>{h.dataset.renderState=previous[i]||'';});body.classList.remove('an-phone-hud');speed.remove();style.remove();return true;}
