...
export function createMobileHud({doc=document,win=window,update,readSpeedKmh=physicalSpeedKmh,now=()=>performance.now(),setTimer=(fn,ms)=>setTimeout(fn,ms),clearTimer=id=>clearTimeout(id)}={}){
 ...
 const style=doc.createElement('link');style.rel='stylesheet';style.href=new URL('../../assets/styles/mobile-hud.css',import.meta.url).href;doc.head.append(style);body.classList.add('an-phone-hud');hosts.forEach(h=>{h.dataset.renderState='mobile';});
