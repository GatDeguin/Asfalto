...
export function installSharedInstanceWindow(T,mesh,{rangeM=500,matrices=null,eligible=null,beforeRender=null}={}){
 ...
 mesh.onBeforeRender=(_renderer,_scene,camera)=>{beforeRender?.();camera.getWorldPosition(cameraLocal);inverse.copy(mesh.matrixWorld).invert();cameraLocal.applyMatrix4(inverse);let count=0,changed=false;
  for(let i=0;i<maximum;i++){const o=i*16;if(eligible&&!eligible(i,source))continue;if((source[o+12]-cameraLocal.x)**2+(source[o+13]-cameraLocal.y)**2+(source[o+14]-cameraLocal.z)**2>rangeM*rangeM)continue;if(packedIndices[count]!==i){for(let k=0;k<16;k++)mesh.instanceMatrix.array[count*16+k]=source[o+k];packedIndices[count]=i;changed=true;}count++;}
 ...
 return{source,replace(next){if(next.length!==source.length)throw new TypeError('Canonical replacement must preserve capacity');source.set(next);packedIndices.fill(-1);canonicalInfo.version++;mesh.instanceMatrix.array.set(source);mesh.count=maximum;mesh.instanceMatrix.clearUpdateRanges();mesh.instanceMatrix.needsUpdate=true;mesh.computeBoundingBox();mesh.computeBoundingSphere();},diagnostics:()=>({maximum,visibleCount})};
}
