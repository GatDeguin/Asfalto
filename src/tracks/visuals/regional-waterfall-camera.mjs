...
function* cameraProfileSteps(T,root,{query,falls,heightAt}){
 ...
 const bounds=new T.Box3();for(const f of chosen)bounds.union(f.box);const center=bounds.getCenter(new T.Vector3()),station=query.project(center.toArray()).sM,targets=[];
 for(const {mesh,box}of chosen){const mid=box.getCenter(new T.Vector3()),impact=mesh.userData.asfaltoWaterfallContact?.impactCenter||mesh.userData.asfaltoWaterfall?.impactCenter;targets.push(mid,new T.Vector3(mid.x,box.max.y-4,mid.z),impact?new T.Vector3(...impact):new T.Vector3(mid.x,box.min.y+4,mid.z));}
