import {buildShorePromontory,shorelineFromGeometry} from './shore-promontory.mjs';
...
export function refineRegionalShoreline(THREE,root,{id,query,heightAt,textures}){
  ...
  for(const water of waters)water.mesh.userData.asfaltoWater={...(water.mesh.userData.asfaltoWater||{}),kind:'lake',maxDepthM:35,shoreWidthM:24,flowDirection:{x:.82,z:.57},flowSpeedMps:0,shoreSegments:water.segments};
  ...
  return {shoreSegments:waters.reduce((n,w)=>n+w.segments.length,0),shoreBanks:banks,shorePromontories:headlands.length};
}
