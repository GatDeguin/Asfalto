...
const phoneMemoryReady=import(new URL('src/runtime/phone-start-memory.mjs?v=928a3aa725daa5ed',document.baseURI));
const startupDemandReady=import(new URL('src/runtime/startup-demand.mjs?v=d7f04a8f3c4b6613',document.baseURI));
const experienceToolsReady=import(new URL('src/menu/v7-session-runtime.mjs?v=4964050ed6c0f11e',document.baseURI)).then(module=>(experienceTools=module));
...
function saveProfile(){renderProfileSummary();profilePersistencePromise||=import(new URL('src/runtime/profile-persistence.mjs?v=34c8f093c015eb86',document.baseURI));return profilePersistencePromise.then(m=>m.saveOrdinaryProfile({storage:globalThis.__asfaltoV7Storage,getProfile:()=>profile,getExpectedRevision:()=>profileExpectedRevision,onCommitted:revision=>profileExpectedRevision=revision})).then(()=>{void syncWorkshopCollection();return true;}).catch(error=>{toast('No se pudo guardar el archivo local: '+String(error.message||error)+'. Exportalo desde Historial.','warn');return false;});}
...
async function loadChampionshipMenu(){
 ...
 return championshipMenuPromise||(championshipMenuPromise=import(new URL('src/menu/v7-championship-menu.mjs?v=c3e627fbb5b121d3',document.baseURI)).then(m=>{
...
async function configureExistingGameReady({track,weather,skyId=profile.lastSky||'clear',mode,laps,difficulty},transaction){
 ...
 await transaction.wait(cockpit.prepareRendering?.());check();
 return true;
}
...
async function loadRoadTestTools(){
 ...
 return roadTestToolsPromise||(roadTestToolsPromise=Promise.all([import(new URL('src/menu/v7-roadtest-session.mjs?v=4450177759448c35',document.baseURI)),import(new URL('src/menu/v7-roadtest-profile.mjs?v=02c72436a82c9342',document.baseURI))]).then(([session,store])=>{
