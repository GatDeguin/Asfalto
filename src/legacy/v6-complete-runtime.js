...
class WorkshopRenderer{
 ...
 async ensureRaceVehicleSelection(){
  ...
  const [{createDeferredRaceVehicle},{getVehicleDefinition}]=await Promise.all([import(new URL('src/menu/deferred-race-vehicle.mjs?v=c256ad346cab3e22',document.baseURI)),import(new URL('src/render/vehicle-catalog.mjs?v=1fb2dbf31facc389',document.baseURI))]);
...
const {createOwnedRenderer}=await import(new URL('src/runtime/owned-renderer.mjs',document.baseURI));
const {disposeAllSync}=await import(new URL('src/runtime/dispose-all.mjs',document.baseURI));
const {snapshotSceneResources}=await import(new URL('src/runtime/scene-resources.mjs',document.baseURI));
