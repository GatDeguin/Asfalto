...
const phoneMemoryReady=import(new URL('src/runtime/phone-start-memory.mjs?v=phone-r6-20260913',document.baseURI));
...
async function waitForExistingGameReady(timeoutMs=180000,{signal,onStage}={}){
 if(workshop.deviceProfile?.phone){onStage?.('Liberando el taller antes de cargar la pista…');await (await phoneMemoryReady).releasePhoneWorkshopForRace(workshop,{signal});}
...
class WorkshopRenderer{
 ...
 setActive(v){if(this.memorySuspended){this.active=false;cancelAnimationFrame(this.raf);this.raf=0;return;}this.active=v;if(!v){this.engine?.suspend();this.chassis?.setActive(false);}if(v&&this.loaded)this.loop();else if(!v){cancelAnimationFrame(this.raf);this.raf=0}}
