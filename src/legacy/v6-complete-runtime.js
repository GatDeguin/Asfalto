...
let experienceTools=null,sessionRequestRunner=null,lastExperienceUiTime=-Infinity;
...
function migrateProfile(value){
 ...
 p.history=Array.isArray(value?.history)?value.history:deepClone(DEFAULT_PROFILE.history);p.sheets=Array.isArray(value?.sheets)?value.sheets:[];p.photos=Array.isArray(value?.photos)?value.photos:[];p.unlocked=Array.from(new Set([...(value?.unlocked||[]),...DEFAULT_PROFILE.unlocked]));p.lastRoute=persistedTrack(value?.lastRoute);return p;
}
...
function saveProfile(){renderProfileSummary();profilePersistencePromise||=import(new URL('src/runtime/profile-persistence.mjs',document.baseURI));return profilePersistencePromise.then(m=>m.saveOrdinaryProfile({storage:globalThis.__asfaltoV7Storage,getProfile:()=>profile,getExpectedRevision:()=>profileExpectedRevision,onCommitted:revision=>profileExpectedRevision=revision})).then(()=>{void syncWorkshopCollection();return true;}).catch(error=>{toast('No se pudo guardar el archivo local: '+String(error.message||error)+'. Exportalo desde Historial.','warn');return false;});}
...
async function loadChampionshipMenu(){
 ...
 return championshipMenuPromise||(championshipMenuPromise=import(new URL('src/menu/v7-championship-menu.mjs',document.baseURI)).then(m=>{
...
async function openChampionship(){q('#v7-championship-content').hidden=false;try{const menu=await loadChampionshipMenu();menu.render();q('#v7-championship-select')?.focus();}catch(error){q('#v7-championship-content').textContent='No se pudo abrir el campeonato. Volvé a elegir Campeonato nacional para reintentar.';}}
...
function loadCollectionReader(){return collectionReaderPromise||(collectionReaderPromise=import(new URL('src/menu/v7-earned-collection.mjs',document.baseURI)).catch(error=>{collectionReaderPromise=null;throw error;}));}
...
function conditionAverage(){return Object.values(profile.condition).reduce((a,b)=>a+b,0)/Object.keys(profile.condition).length}
...
function renderProfileSummary(){
 q('#v6-profile-name').textContent=`${profile.name} · Taller Central`;q('#v6-profile-class').textContent=`Clase ${vehicleClass().toLowerCase()} · ${profile.sheets.length} fichas`;
...
function openPanel(name,focus=true){
 ...
 profile.lastPanel=name;saveProfile();if(name==='workshop')workshop.ensureLoaded();workshop.setActive(['drive','tests','competition','workshop','collection','settings'].includes(name));if(focus)q(`#v6-${name==='tests'?'roadtest':name}-panel h2`)?.focus?.();
...
function publishHomeReadiness(){
 ...
 if(!workshopReady&&!technicalFallback)return false;
 if(!globalThis.__asfaltoLoading?.completeBoot?.({menuReady:true,workshopReady,technicalFallback}))return false;
 app.homeReady=true;app.homeReadyAt=performance.now();app.homeReadyMode=technicalFallback?'technical-fallback':'rendered-workshop';window.dispatchEvent(new CustomEvent('asfalto-home-ready',{detail:{at:app.homeReadyAt,mode:app.homeReadyMode}}));return true;
}
...
function renderAppearance(){
 const wrap=q('#v6-body-swatches');wrap.innerHTML='';for(const [id,c] of Object.entries(BODY_COLORS)){const b=document.createElement('button');b.type='button';b.className='v6-swatch';b.style.setProperty('--swatch',c.hex);b.title=c.label;b.dataset.body=id;b.setAttribute('aria-label',c.label);b.setAttribute('aria-pressed',String(profile.appearance.body===id));b.addEventListener('click',()=>{profile.appearance.body=id;qa('.v6-swatch').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));workshop.setCarColor(c.hex);saveProfile()});wrap.append(b)}workshop.setCarColor(BODY_COLORS[profile.appearance.body]?.hex||'#d66a24');
...
function serviceItem(id){const cost=profile.condition[id]<55?2:1;if(profile.workshopTokens<cost){toast('Faltan fichas de oficio. Completá pruebas válidas.','warn');return}profile.workshopTokens-=cost;profile.condition[id]=Math.min(100,profile.condition[id]+(id==='fuel'?45:30));addHistory(`Servicio: ${CONDITION_META[id].label}`,`Condición llevada a ${Math.round(profile.condition[id])} %.`,'Mantenimiento');award({mecanica:2});renderConditions();saveProfile();toast(`${CONDITION_META[id].label}: servicio completado.`,'ok')}
...
let existingGameReadyPromise=null;
async function waitForExistingGameReady(timeoutMs=180000,{signal,onStage}={}){
 const demand=(await startupDemandReady).startupDemand;await demand.request({signal,onStage});
 if(window.__cockpit?.ready===true)return Promise.resolve(window.__cockpit);
 if(existingGameReadyPromise)return existingGameReadyPromise;
 const started=performance.now();
 toast('PREPARANDO JUEGO...','info');
 existingGameReadyPromise=new Promise(resolve=>{
  const poll=()=>{
   const cockpit=window.__cockpit;
   if(cockpit?.ready===true){resolve(cockpit);return}
   if(cockpit?.error||performance.now()-started>=timeoutMs){
    toast(cockpit?.error?'No se pudo iniciar el cockpit.':'El cockpit demoro demasiado en iniciar.','warn');
    resolve(null);return
   }
   setTimeout(poll,50);
  };
  poll();
 }).finally(()=>{existingGameReadyPromise=null});
 return existingGameReadyPromise;
}
async function getSessionRequestRunner(){
 ...
 if(!sessionRequestRunner)sessionRequestRunner=tools.createSessionTransactionRunner({
  ...
  begin({metadata,cancel,retry}){
   ...
   return {canRetry:!!ui,stage(label,done,total){cinema?.stage(label);ui?.stage(label,done,total);},fail(message){ui?.fail(message);toast(message,'warn');},async end(success){ui?.finish(success?(metadata.editor?'editor':'opening'):'workshop');await cinema?.end();globalThis.__asfaltoV7Experience?.syncPhase();}};
  }
 });
...
async function setChassisConfig(value){
 const m=await import(new URL('src/game/chassis-configuration.mjs',document.baseURI));globalThis.AsfaltoV6Chassis=m;
...
async function loadRoadTestTools(){
 ...
 return roadTestToolsPromise||(roadTestToolsPromise=Promise.all([import(new URL('src/menu/v7-roadtest-session.mjs',document.baseURI)),import(new URL('src/menu/v7-roadtest-profile.mjs',document.baseURI))]).then(([session,store])=>{
...
async function startDrive(mode){
 await leaveChampionship();
 cancelRoadTest();profile.selectedDrive=mode;saveProfile();const requestedTrack=q('#v6-drive-route').value,weather=q('#v6-drive-weather').value;const map={free:'practice',journey:'practice',practice:'practice'};if(!await configureExistingGame({track:requestedTrack,weather,mode:map[mode]||'practice',laps:mode==='journey'?10:99,difficulty:'normal'}))return false;const track=persistPlayableRoute(requestedTrack);applyVehicleConfig();resetSession();activeSession.type=mode;activeSession.definition={id:mode,title:mode==='journey'?'Viaje':'Conducción libre'};activeSession.startTime=performance.now();activeSession.lastTime=activeSession.startTime;activeSession.phase='running';activeSession.config={track,weather};closeMenu();q('#v6-objective-hud').classList.add('v6-visible');updateObjective();startExistingGame();toast(mode==='journey'?'VIAJE INICIADO · cuide temperatura y combustible':'CONDUCCIÓN LIBRE · sin medición activa','ok');sessionLoop.start();return true
}
async function startRoadTest(){
 ...
 try{
  ...
  const success=await runner.run(async tx=>{
   ...
   activeSession.phase=roadTestSession.getState().phase;return tx.isCurrent();
  },{track,roadtest:true});
  if(success){closeMenu();q('#v6-objective-hud').classList.add('v6-visible');updateObjective();toast('PRUEBA PREPARADA · '+def.title,'ok');sessionLoop.start();}
...
async function startCompetition(){await leaveChampionship();cancelRoadTest();const def=COMPETITIONS.find(c=>c.id===profile.selectedCompetition);if(!def)return false;const requestedTrack=q('#v6-comp-track').value,weather=q('#v6-comp-weather').value,difficulty=q('#v6-comp-difficulty').value,laps=clamp(Number(q('#v6-comp-laps').value)||1,1,3);if(!await configureExistingGame({track:requestedTrack,weather,mode:def.mode,laps,difficulty}))return false;const track=persistPlayableRoute(requestedTrack);applyVehicleConfig();resetSession();activeSession.type='competition';activeSession.definition=def;activeSession.startTime=performance.now();activeSession.lastTime=activeSession.startTime;activeSession.phase='running';activeSession.config={track,weather,difficulty,laps,class:vehicleClass()};closeMenu();q('#v6-objective-hud').classList.add('v6-visible');updateObjective();startExistingGame();toast(`COMPETENCIA · ${def.title}`,'ok');sessionLoop.start();return true
...
const sessionLoop={running:false,raf:0,start(){if(this.running)return;this.running=true;const tick=t=>{if(!this.running)return;syncTouchPause();updateSession(t);this.raf=requestAnimationFrame(tick)};this.raf=requestAnimationFrame(tick)},stop(){this.running=false;cancelAnimationFrame(this.raf)}};
...
function updateJourney(s,t,dt){profile.condition.fuel=clamp(profile.condition.fuel-s.speed*dt/52000,0,100);profile.condition.oil=clamp(profile.condition.oil-dt/1600,0,100);profile.condition.tires=clamp(profile.condition.tires-Math.abs(s.lateral)*dt/600,0,100);if(profile.condition.fuel<=2){toast('RESERVA AGOTADA · viaje finalizado','warn');finishJourney(s,t)}if(t-activeSession.startTime>20*60*1000)finishJourney(s,t)}
...
function applyWear(mult=1){profile.condition.engine=clamp(profile.condition.engine-.35*mult,0,100);profile.condition.oil=clamp(profile.condition.oil-.7*mult,0,100);profile.condition.brakes=clamp(profile.condition.brakes-.55*mult,0,100);profile.condition.tires=clamp(profile.condition.tires-.8*mult,0,100);profile.condition.body=clamp(profile.condition.body-(activeSession.valid?.05:.8)*mult,0,100);profile.appearance.dirt=clamp(profile.appearance.dirt+2*mult,0,100)}
...
function setupActions(){
 ...
 q('#v7-championship-open').addEventListener('click',openChampionship);q('#v7-earned-album-open').addEventListener('click',openEarnedAlbum);window.addEventListener('pagehide',()=>earnedAlbum?.dispose());
 ...
 q('#v6-clean-car').addEventListener('click',()=>{profile.appearance.dirt=0;profile.condition.body=clamp(profile.condition.body+3,0,100);addHistory('Limpieza de carrocería','Se retiró polvo de ruta y se revisaron uniones.','Conservación');award({coleccion:1});saveProfile();toast('CARROCERÍA LIMPIA','ok')});
 ...
 q('#v6-service-recommended').addEventListener('click',()=>{const lowest=Object.entries(profile.condition).sort((a,b)=>a[1]-b[1]).slice(0,2);for(const [id] of lowest)if(profile.workshopTokens>0)serviceItem(id)});q('#v6-inspect-all').addEventListener('click',()=>{addHistory('Inspección general',`Condición ${Math.round(conditionAverage())} %. Próximo servicio: ${Object.entries(profile.condition).sort((a,b)=>a[1]-b[1])[0][0]}.`,'Inspección');award({mecanica:3,historia:1});saveProfile();renderHistory();toast('INSPECCIÓN REGISTRADA','ok')});
...
class WorkshopRenderer{
 ...
 bind(){this.canvas.addEventListener('pointerdown',e=>{this.drag={x:e.clientX,y:e.clientY,yaw:this.yaw,pitch:this.pitch};this.canvas.setPointerCapture?.(e.pointerId)});this.canvas.addEventListener('pointermove',e=>{if(!this.drag)return;this.yaw=this.drag.yaw-(e.clientX-this.drag.x)*.006;this.pitch=clamp(this.drag.pitch+(e.clientY-this.drag.y)*.004,-.15,(this.engine?.diagnostics().active||this.chassis?.diagnostics().active) ? 1.55 : 1.1);this.targetPose=null});const end=()=>this.drag=null;this.canvas.addEventListener('pointerup',end);this.canvas.addEventListener('pointercancel',end);this.canvas.addEventListener('wheel',e=>{e.preventDefault();this.radius=clamp((this.camera?this.camera.position.distanceTo(this.target):this.radius)*Math.exp(e.deltaY*.001),(this.engine?.diagnostics().active||this.chassis?.diagnostics().active) ? .55 : 3.3,28);this.targetPose=null},{passive:false});window.addEventListener('resize',()=>this.resize());this.canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();this.failed=true;cancelAnimationFrame(this.raf);this.raf=0;q('#v6-workshop-fallback').classList.remove('v6-hidden');toast('El contexto 3D del taller se perdió. Se mantiene el menú técnico.','warn')});this.canvas.addEventListener('webglcontextrestored',async()=>{this.restoring=true;await this.loadingTask;this.releaseScene();this.failed=false;this.loading=false;this.restoring=false;this.ensureLoaded()})}
 releaseScene(){earnedAlbum?.releasePosters();cancelAnimationFrame(this.raf);this.raf=0;this.advancedGraphics?.dispose();this.advancedGraphics=null;this.chassis?.dispose();this.chassis=null;this.vehicleSelector?.dispose();this.vehicleSelector=null;this.rayTracing?.dispose();this.rayTracing=null;this.engine?.dispose();this.engine=null;this.inspection?.dispose();this.inspection=null;this.vehiclePresentation?.dispose();this.vehiclePresentation=null;this.presentation?.dispose();this.presentation=null;this.cameraConstraint=null;this.key?.shadow?.dispose();for(const resource of new Set(this.resources)){resource.dispose?.();resource.close?.()}this.resources=[];this.renderer?.dispose();this.renderer=null;this.scene=null;this.car=null;this.workshopRoot=null;this.loaded=false}
 setActive(v){this.active=v;if(!v){this.engine?.suspend();this.chassis?.setActive(false);}if(v&&this.loaded)this.loop();else if(!v){cancelAnimationFrame(this.raf);this.raf=0}}
 ensureLoaded(){if(this.loaded||this.failed||this.restoring)return this.loadingTask;if(this.loading)return this.loadingTask;this.loadingTask=this.loadScene();return this.loadingTask}
 async loadScene(){this.loading=true;const loading=q('#v6-workshop-loading');const stage=globalThis.__asfaltoLoading.workshop(loading);const bar=q('.v6-progress>i',loading);loading.classList.remove('v6-hidden');try{stage('Preparando el taller…');bar.style.width='4%';this.T=await loadThreeModule();stage('Cargando paredes, herramientas y materiales…');bar.style.width='12%';const node=q('#v6-workshop-payload');if(!node)throw new Error('Modelo del taller no encontrado');const glb=node.dataset.encoding === 'external-url' ? (await globalThis.AsfaltoV6AssetCore.readExternalPayload(node)).buffer : await decodeGzipBase64(node.textContent,p=>bar.style.width=`${12+p*62}%`);stage('Montando el auto, luces y reflejos…');bar.style.width='76%';await this.initScene(glb,p=>bar.style.width=`${76+p*22}%`);this.loaded=true;this.loading=false;stage('Taller listo');loading.setAttribute('aria-busy','false');stage.dispose?.();bar.style.width='100%';setTimeout(()=>{loading.classList.add('v6-hidden');publishHomeReadiness();},450);q('#v6-workshop-fallback').classList.add('v6-hidden');this.loop();void this.vehicleSelector?.restore();toast('TALLER MECÁNICO 1973 CARGADO','ok')}catch(err){console.error('[V6 workshop]',err);this.releaseScene();this.failed=true;this.loading=false;loading.setAttribute('aria-busy','false');stage.dispose?.();loading.classList.remove('an-cinema-loading');loading.textContent=`Vista técnica activa · ${err.message}`;q('#v6-workshop-fallback').classList.remove('v6-hidden');publishHomeReadiness();}}
 async initScene(glb,onProgress){const T=this.T;this.renderer=new T.WebGLRenderer({canvas:this.canvas,antialias:true,alpha:false,powerPreference:'high-performance',preserveDrawingBuffer:true});this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,innerWidth<720?1.25:1.75));this.renderer.toneMapping=T.ACESFilmicToneMapping??0;this.renderer.toneMappingExposure=1.06;if('outputColorSpace'in this.renderer)this.renderer.outputColorSpace=T.SRGBColorSpace;this.scene=new T.Scene();this.scene.background=new T.Color(0x111416);this.scene.fog=new T.FogExp2(0x111416,.018);this.camera=new T.PerspectiveCamera(46,1,.05,120);this.hemi=new T.HemisphereLight(0xc9d7dd,0x342418,1.45);this.scene.add(this.hemi);this.key=new T.DirectionalLight(0xffe2bd,3.2);this.key.position.set(7,12,5);this.scene.add(this.key);this.fill=new T.DirectionalLight(0x9ebcd2,1.15);this.fill.position.set(-8,7,-7);this.scene.add(this.fill);this.warm=new T.PointLight(0xff8b42,2.8,18,1.5);this.warm.position.set(4,3,-3);this.scene.add(this.warm);
 const parsed=this.parseGLB(glb),root=await this.buildGLTF(parsed,onProgress);this.scene.add(root);const box=new T.Box3().setFromObject(root),size=new T.Vector3(),center=new T.Vector3();box.getSize(size);box.getCenter(center);const max=Math.max(size.x,size.z,size.y*.7)||1,scale=18/max;root.scale.set(scale,scale*.8,scale);root.position.set(-center.x*scale,-box.min.y*scale*.8,-center.z*scale);root.rotation.y=Math.PI;root.updateMatrixWorld(true);const rotatedBox=new T.Box3().setFromObject(root),rotatedCenter=new T.Vector3();rotatedBox.getCenter(rotatedCenter);root.position.x-=rotatedCenter.x;root.position.y-=rotatedBox.min.y;root.position.z-=rotatedCenter.z;root.updateMatrixWorld(true);this.workshopRoot=root;this.addChevyV3Car();const appearance=await import(new URL('src/render/workshop-presentation.mjs',document.baseURI));await appearance.enhanceWorkshop(this);const {createVehiclePresentation}=await import(new URL('src/render/vehicle-presentation.mjs',document.baseURI));this.vehiclePresentation=await createVehiclePresentation(T,{vehicle:'chevy',modelRoot:this.car,loadGlb:window.__asfaltoLoadVehicleModel,paintColor:this.paintColor||globalThis.__chevyPaintColor||'#d66a24'});const exteriorBounds=new T.Box3().setFromObject(this.vehiclePresentation.root);this.car.position.y+=(this.car.userData.surfaceY||0)-exteriorBounds.min.y;this.car.userData.floorY=this.car.position.y;this.vehiclePresentation.setEnvironment(this.presentation.getRoomEnvironment());const {createWorkshopInspection}=await import(new URL('src/render/workshop-inspection.mjs',document.baseURI));this.inspection=createWorkshopInspection(T,this);const {createWorkshopEngine}=await import(new URL('src/render/workshop-engine.mjs',document.baseURI));this.engine=createWorkshopEngine(T,this);const {createWorkshopChassis}=await import(new URL('src/render/workshop-chassis.mjs',document.baseURI));this.chassis=createWorkshopChassis(T,this);const {createWorkshopCameraConstraint}=await import(new URL('src/render/workshop-camera-constraint.mjs',document.baseURI));this.cameraConstraint=createWorkshopCameraConstraint(T,root);const {createRayTracedOcclusion}=await import(new URL('src/render/ray-traced-occlusion.mjs',document.baseURI));this.rayTracing=createRayTracedOcclusion({THREE:T,renderer:this.renderer,scene:this.scene,camera:this.camera,excludeOccluders:()=>[this.car,this.engine?.mount],mode:globalThis.__asfaltoRayTracing?.getMode()||'off'});this.resize();this.focus('general',true);globalThis.__asfaltoMenuPresentation?.configureWorkshop(this);const {installWorkshopVehicleSelector}=await import(new URL('src/menu/workshop-vehicle-selector.mjs',document.baseURI));const {prepareWorkshopVehicle}=await import(new URL('src/render/workshop-vehicle-selection.mjs',document.baseURI));const {createDeferredRaceVehicle}=await import(new URL('src/menu/deferred-race-vehicle.mjs',document.baseURI));this.raceVehicleSelection=createDeferredRaceVehicle();this.vehicleSelector=installWorkshopVehicleSelector({workshop:this,prepareWorkshop:id=>prepareWorkshopVehicle(T,this,id),prepareRace:id=>this.raceVehicleSelection.prepare(id)});const {createAdvancedGraphics}=await import(new URL('src/render/advanced-graphics.mjs',document.baseURI));this.advancedGraphics=createAdvancedGraphics(T,{renderer:this.renderer,scene:this.scene,camera:this.camera,scope:'workshop',getEnvironment:()=>({skyId:this.presentation?.diagnostics().lighting==='night'?'night':'clear',weather:'clear',keyLightDirection:this.key.position.toArray(),keyLightColor:'#'+this.key.color.getHexString(),sunIntensity:this.key.intensity})});}
...
function initV6(){
 ...
 const begin=()=>{ambient.start();workshop.ensureLoaded();document.removeEventListener('pointerdown',begin)};document.addEventListener('pointerdown',begin,{once:true});setTimeout(()=>workshop.ensureLoaded(),1200);
