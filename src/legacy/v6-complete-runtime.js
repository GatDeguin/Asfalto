...
async function getSessionRequestRunner(){
 ...
 if(!sessionRequestRunner)sessionRequestRunner=tools.createSessionTransactionRunner({
  ...
  begin({metadata,cancel,retry}){
   ...
   return {canRetry:!!ui,stage(label,done,total){cinema?.stage(label);ui?.stage(label,done,total);},fail(message){ui?.fail(message);toast(message,'warn');},async end(success){try{ui?.finish(success?(metadata.editor?'editor':'opening'):'workshop');await cinema?.end();}finally{globalThis.__asfaltoRacePresentationHeld=false;}globalThis.__asfaltoV7Experience?.syncPhase();}};
  }
 });
...
async function openPreparedRace(tx){
 ...
 await tx.wait(new Promise(resolve=>requestAnimationFrame(()=>{window.__cockpit.prepareRacePresentation();requestAnimationFrame(resolve);})));return tx.isCurrent();
}
...
async function initV6(){
 ...
 const {createVehicleMaintenance}=await import(new URL('src/game/vehicle-maintenance.mjs',document.baseURI));
 const {mountVehicleAssistance}=await import(new URL('src/menu/v7-vehicle-assistance.mjs',document.baseURI));
 ...
 const restoreVehiclePaint=id=>{profile.vehiclePaintV7||={};const hex=profile.vehiclePaintV7[id]||(id==='chevy_400_1957'?'#b91518':workshop.paintColor||globalThis.__chevyPaintColor||'#d66a24');profile.vehiclePaintV7[id]=hex;workshop.setCarColor(hex);};
