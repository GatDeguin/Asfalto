...
const phoneMemoryReady=import(new URL('src/runtime/phone-start-memory.mjs?v=balance-20260917',document.baseURI));
...
async function loadChampionshipMenu(){
 ...
 return championshipMenuPromise||(championshipMenuPromise=import(new URL('src/menu/v7-championship-menu.mjs?v=balance-20260917',document.baseURI)).then(m=>{
...
function loadCollectionReader(){return collectionReaderPromise||(collectionReaderPromise=import(new URL('src/menu/v7-earned-collection.mjs?v=balance-20260917',document.baseURI)).catch(error=>{collectionReaderPromise=null;throw error;}));}
...
async function loadRoadTestTools(){
 ...
 return roadTestToolsPromise||(roadTestToolsPromise=Promise.all([import(new URL('src/menu/v7-roadtest-session.mjs?v=balance-20260917',document.baseURI)),import(new URL('src/menu/v7-roadtest-profile.mjs?v=balance-20260917',document.baseURI))]).then(([session,store])=>{
...
class WorkshopRenderer{
 ...
 setActive(v){if(this.memorySuspended){this.active=false;cancelAnimationFrame(this.raf);this.raf=0;return;}if(v&&!this.active)this.advancedGraphics?.beginFrameWindow();this.active=v;if(!v){this.engine?.suspend();this.chassis?.setActive(false);}if(v&&this.loaded)this.loop();else if(!v){cancelAnimationFrame(this.raf);this.raf=0}}
...
async function initV6(){
 ...
 const {createVehicleMaintenance}=await import(new URL('src/game/vehicle-maintenance.mjs?v=balance-20260917',document.baseURI));
