...
export function createDeferredRaceVehicle({getCockpit=()=>globalThis.__cockpit}={}){let selected='chevy',applied='chevy';
 ...
 async function ensureApplied(cockpit,{signal}={}){while(selected!==applied){const id=selected,stage=await cockpit.preparePlayerVehicle(id);if(signal?.aborted){stage.dispose();throw signal.reason;}if(id!==selected){stage.dispose();continue;}stage.commit();applied=id;}return applied;}
