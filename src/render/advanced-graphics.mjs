import {createAdvancedMaterials}from'./advanced-materials.mjs?v=0803bce77b39a217';
import {createPivotPainter}from'./pivot-painter.mjs?v=52e6d9dc8ec9023d';
import {createDistanceFieldOcclusion}from'./distance-field-occlusion.mjs?v=84da235ddae579a2';
import {createPhysicalAtmosphere}from'./physical-atmosphere.mjs?v=95e486b6e674c928';
import {createScreenSpaceLighting,screenLightingPolicy}from'./screen-space-lighting.mjs?v=fd4d6e73c4813c17';
import {readAdvancedGraphics,normalizeAdvancedGraphics,effectiveGraphicsQuality}from'./advanced-graphics-settings.mjs?v=166328ee3596134a';
import {setSurfaceReliefPDO,surfaceReliefDiagnostics}from'../tracks/visuals/surface-relief.mjs?v=cd94870ee4671a7f';
export function createAdvancedGraphics(T,{renderer,scene,camera,scope='world',getEnvironment=()=>({}),getQuality=()=> 'high'}={}){
 ...
 const pivot=createPivotPainter(T,{root:scene,quality});pivot.refresh();const materials=createAdvancedMaterials(T,{root:scene,scope,quality}),field=createDistanceFieldOcclusion(T,{scene}),atmosphere=createPhysicalAtmosphere(T,{scene,scope,quality}),post=createScreenSpaceLighting(T,{renderer,scene,camera,distanceField:field,atmosphere,quality});
 function configure(){
