import {AWARDS} from '../menu/v7-championship-catalog.mjs';
...
export function createWorkshopCollection(T,{textures={},collection=[],posters=[]}={}){
 ...
 const material=(name,color,roughness,metalness,map)=>{const m=new T.MeshStandardMaterial({color,roughness,metalness,map:map||null});m.name='Collection_'+name;owned.materials.add(m);return m;};
 const wood=material('DarkOiledWood','#35261d',.8,.02,textures.wood),back=material('RecessedWood','#201c17',.93,0,textures.back||textures.wood),bronze=material('PatinatedBronze','#82704e',.61,.7),gold=material('WarmBrass','#baa06a',.43,.78),silver=material('SatinNickel','#b0b6b4',.47,.77),copper=material('Copper','#aa7451',.61,.62),green=material('DeepGreenEnamel','#315d4f',.48,.18),water=material('MatteWaterGlass','#6b9290',.74,.10),stone=material('StoneBase','#5e5950',.88,0);
 ...
 const cylinder=(n=16)=>primitive('cyl'+n,()=>new T.CylinderGeometry(1,1,1,n,1));
 const cone=()=>primitive('cone',()=>new T.ConeGeometry(1,1,8,1));
 const ring=(arc=Math.PI*2)=>primitive('ring'+arc,()=>new T.TorusGeometry(1,.08,4,24,arc));
 ...
 AWARDS.forEach((a,i)=>{const p=positionFor(a,i);slots.push({id:a.id,kind:a.kind,position:[...p],heightM:a.heightM});if(a.kind==='test-medal'){cabinet.add(cylinder(6),bronze,[p[0],p[1]+.065,.135],[.003,.035,.003],[Math.PI/2,0,0]);}else cabinet.add(box(),bronze,[p[0],p[1]-.006,.045],[a.kind==='cup'?.31:.29,.006,.22]);});
 cabinet.finish();
 ...
 function awardGeometry(a,index){const group=new T.Group();group.name='Award_'+a.id;group.userData={awardId:a.id,shape:a.shape,heightM:a.heightM,earned:false};const b=batch(group,a.id,.002/a.heightM);const main=a.material.includes('silver')||a.material.includes('nickel')||a.material.includes('aluminium')?silver:a.material.includes('copper')?copper:a.kind==='cup'&&a.championshipId==='gran_nacional'?gold:bronze;
  if(a.kind==='test-medal'){
   ...
   if(a.kind==='route-trophy'){
    ...
    if(c===1){const points=[[.08,.14],[.045,.42],[.13,.48],[.25,.65],[.29,.87],[.275,.91]].map(([x,y])=>new T.Vector2(x,y));const g=new T.LatheGeometry(points,20);b.add(g,main);g.dispose();for(const side of [-1,1])b.add(ring(Math.PI*1.7),main,[side*.28,.65,0],[.15,.24,.15],[0,0,side<0?-.9:2.3]);b.add(ring(),gold,[0,.89,0],[.28,.28,.28],[Math.PI/2,0,0]);}
