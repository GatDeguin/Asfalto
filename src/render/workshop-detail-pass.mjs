...
export function createWorkshopDetailPass(T, workshop, {floorY=.188, makeLabel=labelTexture}={}) {
  ...
  // Skirting, ceiling returns and the original side door jambs.
  box([0,floorY+.085,-5.81],[17.35,.17,.08],dark);
  for(const x of [-8.65,8.65])box([x,floorY+.085,.04],[.08,.17,11.68],dark);
  for(const x of [-8.61,8.61])box([x,4.64,.04],[.13,.11,11.68],cream);
  box([0,4.64,-5.76],[17.22,.11,.13],cream);
  for(const z of [3.14,4.53])box([8.61,1.26,z],[.12,2.16,.07],steel);
  box([8.61,2.33,3.83],[.12,.09,1.48],steel);
  ...
  // A tool board: hanging hooks and deliberately spaced service tools.
  box([-5.62,1.95,-5.765],[2.1,1.25,.06],material('Pegboard','#575d48',.96));
  for(let row=0;row<6;row++)for(let col=0;col<10;col++)cylinder([-6.51+col*.20,1.44+row*.19,-5.724],.012,.006,dark,[Math.PI/2,0,0]);
  for(let i=0;i<7;i++) {
    const x=-6.33+i*.24,y=2.39-(i%2)*.22,z=-5.66;
  ...
  label('BANCO / HERRAMIENTAS',[-5.63,2.71,-5.70],[2.02,.20]);
  ...
  box([7.18,1.627,-5.12],[.57,.026,.38],trayMat);
  for(const x of [6.895,7.465])box([x,1.655,-5.12],[.016,.065,.40],trayMat);
  for(const z of [-5.31,-4.93])box([7.18,1.655,z],[.59,.065,.016],trayMat);
  for(let i=0;i<14;i++) {
    const x=7.01+(i%5)*.073,z=-5.24+Math.floor(i/5)*.068;
    cylinder([x,1.654,z],.014,.021,steel);box([x,1.644,z+.02],[.012,.011,.065],steel,[0,.3+i*.23,0]);
  }
  tube([[7.13,1.674,-5.17],[7.21,1.674,-5.20],[7.29,1.674,-5.14],[7.22,1.674,-5.06],[7.13,1.674,-5.17]],.009,dark);
  ...
  ragGeo.computeVertexNormals();owned.add(ragGeo);const rag=new T.Mesh(ragGeo,cloth);rag.rotation.x=-Math.PI/2;rag.position.set(6.78,1.64,-5.17);rag.castShadow=true;group.add(rag);
  // Cable hangs from socket; air hose rests on concrete, with connector.
  tube([[6.6,1.15,-5.69],[6.56,.58,-5.59],[6.46,floorY+.03,-5.22],[5.75,floorY+.022,-4.69],[5.12,floorY+.023,-4.07],[4.88,floorY+.025,-3.59]],.012,dark);
  box([6.6,1.19,-5.73],[.075,.12,.042],cream);
  cylinder([4.88,floorY+.03,-3.59],.022,.11,steel,[Math.PI/2,0,.8]);
  ...
  for(let i=0;i<3;i++) {
    const x=-.23+i*.21,y=.826,z=-5.32,h=.19+i*.028,r=.064;
  ...
  const api={
    group,
    setNight(value){night=!!value;for(const l of lights)l.intensity=night?140:22;for(const m of emitters){m.emissiveIntensity=night?3.2:1.1;}for(const m of windows)m.emissiveIntensity=night?.015:.42;dustMat.uniforms.power.value=night?.05:1;return night;},
...
float wsNoise(vec3 p){vec3 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);return mix(mix(mix(wsHash(i),wsHash(i+vec3(1,0,0)),f.x),mix(wsHash(i+vec3(0,1,0)),wsHash(i+vec3(1,1,0)),f.x),f.y),mix(mix(wsHash(i+vec3(0,0,1)),wsHash(i+vec3(1,0,1)),f.x),mix(wsHash(i+vec3(0,1,1)),wsHash(i+vec3(1,1,1)),f.x),f.y),f.z);}`);
    ...
    if(kind==='floor')code+='vec2 service=(wp.xz-vec2(1.,-.65))*vec2(.7,1.1);float stain=exp(-dot(service,service))*smoothstep(.23,.76,mottled);float edge=exp(-abs(abs(wp.x)-8.65)*3.)+exp(-abs(wp.z+5.8)*3.);float track=(1.-smoothstep(.10,.20,abs(abs(wp.z+.3)-.85)))*(1.-smoothstep(2.8,4.9,abs(wp.x)));diffuseColor.rgb*=mix(1.,.47,stain)*(.94-min(.14,edge*.1)-track*.07);';
    ...
    if(kind==='floor')shader.fragmentShader=shader.fragmentShader.replace('#include <roughnessmap_fragment>','#include <roughnessmap_fragment>\nroughnessFactor=mix(roughnessFactor,.28,stain*.8);');
  };
  material.customProgramCacheKey=()=>`workshop-patina-2-${kind}-${seed}`;material.needsUpdate=true;
}
