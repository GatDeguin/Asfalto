...
export function readAdvancedGraphics(storage=globalThis.localStorage){try{return normalizeAdvancedGraphics(JSON.parse(storage?.getItem(ADVANCED_GRAPHICS_KEY)||'{}'));}catch{return{...DEFAULT_ADVANCED_GRAPHICS};}}
...
export function installAdvancedGraphicsSettings({root=document,storage=globalThis.localStorage,getDiagnostics=()=>null}={}){
 ...
 function refresh(){if(disposed)return;const d=getDiagnostics(),quality=d?.effectiveQuality||settings.quality;const label={auto:'Adaptativo',high:'Alto',balanced:'Equilibrado',low:'Bajo',off:'Apagado'}[quality];const text=quality==='off'?'Realismo avanzado desactivado.':label+' · materiales e iluminación'+(quality==='low'?' con efectos reducidos.':' con oclusión y rebotes de luz.');if(text===last)return;last=text;for(const s of statuses)s.textContent=text;}
